...
option_u8 parse_uint8_fastswar_option(const char *str, size_t len) {
  ...
  uint32_t all_digits =
      ((digits.as_int | (0x06060606 + digits.as_int)) & 0xF0F0F0F0) == 0;
  uint8_t num = (uint8_t)((0x640a01 * digits.as_int) >> 24);
  return {all_digits && ((__builtin_bswap32(digits.as_int) <= 0x020505)), num};
}
...
option_u8 parse_uint8_fastswar_bob_option(const char *str, size_t len) {
  ...
  digits.as_int ^= 0x303030lu;
  digits.as_int <<= (len ^ 3) * 8;
  uint8_t num = (uint8_t)((0x640a01 * digits.as_int) >> 16);
  return {
    ((((digits.as_int + 0x767676) | digits.as_int) & 0x808080) == 0) &&
    ((len ^ 3) < 3) && __builtin_bswap32(digits.as_int) <= 0x020505ff,
    num
  };
}
